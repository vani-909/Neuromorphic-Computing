...
class HardwareAdam(torch.optim.Optimizer):
    ...
    # Slop calculation for gain and drift
    @staticmethod
    def _poly_slope(w_sub, key, noise_clip):
        if key not in results or w_sub.numel() == 0:
            return torch.zeros_like(w_sub)

        # Fitted polynomial coefficients (a, b, c) and cov.
        _, popt_np, pcov_np = results[key]
        popt = torch.tensor(popt_np, dtype=torch.float32, device=w_sub.device)
        pcov = torch.tensor(pcov_np, dtype=torch.float32, device=w_sub.device)
    ...
    def _poly_gain(w, step, *, pulse, v_amp, max_gain, noise_clip):
        ...
        def _gain(mask, key):
            ...
            slope = HardwareAdam._poly_slope(w_flat[mask], key, noise_clip)
    ...
    # Passive drift helper: Δw after elapsed idle time `dt`
    @staticmethod
    def _poly_drift(w, dt, *, v_amp, noise_clip):
        w_flat = w.reshape(-1)
        drift  = torch.zeros_like(w_flat)
        ...
        if pos_mask.any():
            slope_pos = HardwareAdam._poly_slope(w_flat[pos_mask], f"0{ v_amp}", noise_clip)
            drift[pos_mask] = slope_pos * dt   # Δw = slope · Δt

        if neg_mask.any():
            slope_neg = HardwareAdam._poly_slope(w_flat[neg_mask], f"0{-v_amp}", noise_clip)
            drift[neg_mask] = slope_neg * dt

        drift[~torch.isfinite(drift) | (drift.abs() > noise_clip)] = 0.0    # Sanity Check
        return drift.reshape_as(w)


    # Optimiser step 
    def step(self, closure=None):
        loss = closure() if closure is not None else None
        now  = time.time()                           # wall-clock for drift
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if len(state) == 0:
                    ...
                    state['t_last_write'] = now     

                # Add elapsed-time drift before applying gradient
                dt = now - state['t_last_write']
                if dt > 0:
                    drift = self._poly_drift(p.data, dt, v_amp=v_amp, noise_clip=noise_clip)
                    p.data.add_(drift)
                ...
                delta_w = self._poly_gain(p.data, step_tensor, pulse=pulse,
                                          v_amp=v_amp, max_gain=max_gain, noise_clip=noise_clip)

                # Parameter update
                p.data.add_(delta_w, alpha=-lr)

                # record time of this write for the next drift calc
                state['t_last_write'] = now
...
# 6. Ensuring Reproducability of results
def reproducability_setup(seed=42):
    random.seed(seed)
    np.random.seed(seed)
...
for PULSE, OPERATING_VOLTAGE in hw_configs(results):
            ...
            if done:
                ...
                if len(episode_durations) >= 100:
                    ...
                    if avg100 > best_score:
                        ...
                        model_path = f"Best Models/best_{PULSE}_{OPERATING_VOLTAGE:+.2f}V.pt"
    ...
    # Save it's video
    video_filename = f"Videos/CartPole-v1_{PULSE}_{OPERATING_VOLTAGE:+.2f}V.mp4"
    os.makedirs(os.path.dirname(video_filename), exist_ok=True)
    record_video(env, video_filename)

    # Save it's training plot
    plt.figure(1)
    os.makedirs("Plots", exist_ok=True)
    plt.savefig(f"Plots/Training_{PULSE}_{OPERATING_VOLTAGE:+.2f}V.png")

    # Training logs are saved as csv above
